    ...
    for i in range(args.checkpoint_start, args.checkpoint_end):
        ...
        if rank == 0:
            ...
            with open(args.out, 'w') as f:
                json.dump(check, f)
            stats = validate('datasets/CityPersons/val_gt.json', args.out)
            MRs = stats["lamr"]
            if traditional_test:
            ...
            with open(args.out, 'w') as f:
                json.dump(check, f)
            stats = validate('datasets/CityPersons/train_vis.json', args.out)
            MRs = stats["lamr"]
            if traditional_test:
                print("Checkpoint %d: [VR: %.2f], [VS: %.2f], [VH: %.2f], [VA: %.2f]" % (
                i, MRs[0], MRs[1], MRs[2], MRs[3]))
                tval_writer.add_scalar('Reasonable', MRs[0], i)
                tval_writer.add_scalar('Small', MRs[1], i)
                tval_writer.add_scalar('Heavy', MRs[2], i)
                tval_writer.add_scalar('All', MRs[3], i)
